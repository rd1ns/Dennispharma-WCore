...
namespace WCore.Services.Vendors
{
    ...
    /// Vendor service
    /// </summary>
    public partial class VendorService : Repository<Vendor>, IVendorService
    {
        #region Fields
        ...
        public VendorService(WCoreContext context, IEventPublisher eventPublisher,
            IRepository<Product> productRepository,
            IRepository<Vendor> vendorRepository,
            IRepository<VendorNote> vendorNoteRepository) : base(context)
        {
        ...
        public virtual Vendor GetVendorByProductId(int productId)
        {
            ...
            return (from v in _vendorRepository.GetAll()
                    join p in _productRepository.GetAll() on v.Id equals p.VendorId
        ...
        /// <returns>Vendors</returns>
        public virtual IPagedList<Vendor> GetAllByFilters(string name = "", string email = "", int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            var data = query.OrderBy(v => v.DisplayOrder).ThenBy(v => v.Name).ThenBy(v => v.Email).ToList();

            var vendors = new PagedList<Vendor>(data, pageIndex, pageSize);
            return vendors;
        }
        ...
        {
            var query = _vendorNoteRepository.GetAll().Where(vn => vn.VendorId == vendorId);

            var data = query.OrderBy(v => v.CreatedOn).ThenBy(v => v.Id).ToList();

            return new PagedList<VendorNote>(data, pageIndex, pageSize);
        }
