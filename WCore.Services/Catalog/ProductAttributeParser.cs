...
namespace WCore.Services.Catalog
{
    ...
    public partial class ProductAttributeParser : IProductAttributeParser
    {
        ...
        protected virtual IList<int> ParseProductProductAttributeIds(string attributesXml)
        {
        ...
        protected IList<Tuple<string, string>> ParseValuesWithQuantity(string attributesXml, int productProductAttributeId)
        {
            ...
            try
            {
                ...
                foreach (XmlNode attributeNode in xmlDoc.SelectNodes(@"//Attributes/ProductAttribute"))
                {
                    ...
                    if (!int.TryParse(attributeNode.Attributes["ID"].InnerText.Trim(), out var attributeId) ||
                        attributeId != productProductAttributeId)
                        continue;
        ...
        protected virtual string GetProductAttributesXml(Product product, IFormCollection form, List<string> errors)
        {
            ...
            var productAttributes = _productAttributeService.GetProductProductAttributesByProductId(product.Id);
            foreach (var attribute in productAttributes)
            {
                var controlId = $"{WCoreCatalogDefaults.ProductAttributePrefix}{attribute.Id}";
                    ...
                    case AttributeControlType.ImageSquares:
                        {
                            ...
                            if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                            {
                                ...
                                if (selectedAttributeId > 0)
                                {
                                    ...
                                    var quantity = 1;
                                    var quantityStr = form[$"{WCoreCatalogDefaults.ProductAttributePrefix}{attribute.Id}_{selectedAttributeId}_qty"];
                            ...
                            if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                            {
                                    ...
                                    .Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                                {
                                    ...
                                    if (selectedAttributeId > 0)
                                    {
                                        //get quantity entered by user
                                        var quantity = 1;
                                        var quantityStr = form[$"{WCoreCatalogDefaults.ProductAttributePrefix}{attribute.Id}_{item}_qty"];
                                        if (!StringValues.IsNullOrEmpty(quantityStr) &&
                                            (!int.TryParse(quantityStr, out quantity) || quantity < 1))
                                            errors.Add(_localizationService.GetResource("Products.QuantityShouldBePositive"));
        ...
        public virtual IList<ProductProductAttribute> ParseProductProductAttributes(string attributesXml)
        {
            var result = new List<ProductProductAttribute>();
            if (string.IsNullOrEmpty(attributesXml))
                return result;

            var ids = ParseProductProductAttributeIds(attributesXml);
            foreach (var id in ids)
            {
                var attribute = _productAttributeService.GetProductProductAttributeById(id);
                if (attribute != null)
                {
        ...
        /// </summary>
        /// <param name="attributesXml">Attributes in XML format</param>
        /// <param name="productProductAttributeId">Product attribute mapping identifier; pass 0 to load all values</param>
        /// <returns>Product attribute values</returns>
        public virtual IList<ProductAttributeValue> ParseProductAttributeValues(string attributesXml, int productProductAttributeId = 0)
        {
            var values = new List<ProductAttributeValue>();
            if (string.IsNullOrEmpty(attributesXml))
                return values;

            var attributes = ParseProductProductAttributes(attributesXml);

            //to load values only for the passed product attribute mapping
            if (productProductAttributeId > 0)
                attributes = attributes.Where(attribute => attribute.Id == productProductAttributeId).ToList();
            ...
            foreach (var attribute in attributes)
            {
                ...
                foreach (var attributeValue in ParseValuesWithQuantity(attributesXml, attribute.Id))
                {
                    ...
                    if (!string.IsNullOrEmpty(attributeValue.Item2) && int.TryParse(attributeValue.Item2, out var quantity) && quantity != value.Quantity)
                    {
                        ...
                        var oldValue = _productAttributeValueRepository.GetById(value);

                        oldValue.ProductProductAttributeId = attribute.Id;
        ...
        /// <param name="productProductAttributeId">Product attribute mapping identifier</param>
        /// <returns>Product attribute values</returns>
        public virtual IList<string> ParseValues(string attributesXml, int productProductAttributeId)
        {
            ...
            try
            {
                ...
                foreach (XmlNode node1 in nodeList1)
                {
                    ...
                    if (id != productProductAttributeId)
                        continue;
        ...
        public virtual string AddProductAttribute(string attributesXml, ProductProductAttribute productProductAttribute, string value, int? quantity = null)
        {
            ...
            try
            {
                ...
                foreach (XmlNode node1 in nodeList1)
                {
                    ...
                    if (id != productProductAttribute.Id)
                        continue;
                ...
                if (attributeElement == null)
                {
                    ...
                    attributeElement.SetAttribute("ID", productProductAttribute.Id.ToString());
        ...
        /// <param name="productProductAttribute">Product attribute mapping</param>
        /// <returns>Updated result (XML format)</returns>
        public virtual string RemoveProductAttribute(string attributesXml, ProductProductAttribute productProductAttribute)
        {
            var result = string.Empty;
            try
            {
        ...
        {
            var attributes1 = ParseProductProductAttributes(attributesXml1);
            if (ignoreNonCombinableAttributes)
            {
                attributes1 = attributes1.Where(x => !x.IsNonCombinable()).ToList();
            }

            var attributes2 = ParseProductProductAttributes(attributesXml2);
            if (ignoreNonCombinableAttributes)
            {
        ...
        /// <returns>Result</returns>
        public virtual bool? IsConditionMet(ProductProductAttribute pam, string selectedAttributesXml)
        {
            if (pam == null)
            ...
            var dependOnAttribute = ParseProductProductAttributes(conditionAttributeXml).FirstOrDefault();
        ...
        public virtual IList<string> GenerateAllCombinations(Product product, bool ignoreNonCombinableAttributes = false, IList<int> allowedAttributeIds = null)
        {
            ...
            var allProductAttributMappings = _productAttributeService.GetProductProductAttributesByProductId(product.Id);
            if (ignoreNonCombinableAttributes)
            {
            ...
            foreach (var combination in allPossibleAttributeCombinations)
            {
                ...
                foreach (var productProductAttribute in combination)
                {
                    if (!productProductAttribute.ShouldHaveValues())
                        continue;

                    //get product attribute values
                    var attributeValues = _productAttributeService.GetProductAttributeValues(productProductAttribute.Id);
                    ...
                    var isCheckbox = productProductAttribute.AttributeControlType == AttributeControlType.Checkboxes ||
                                     productProductAttribute.AttributeControlType ==
                    ...
                    if (isCheckbox)
                    {
                        ...
                        foreach (var oldXml in attributesXml.Any() ? attributesXml : new List<string> { string.Empty })
                        {
                            foreach (var checkboxCombination in CreateCombination(attributeValues))
                            {
                                ...
                                {
                                    newXml = AddProductAttribute(newXml, productProductAttribute, checkboxValue.Id.ToString());
                                }
                    ...
                    else
                    {
                        ...
                        foreach (var oldXml in attributesXml.Any() ? attributesXml : new List<string> { string.Empty })
                        {
                            currentAttributesXml.AddRange(attributeValues.Select(attributeValue =>
                                AddProductAttribute(oldXml, productProductAttribute, attributeValue.Id.ToString())));
                        }
                    }
        ...
        public virtual decimal ParseUserEnteredPrice(Product product, IFormCollection form)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            if (form == null)
