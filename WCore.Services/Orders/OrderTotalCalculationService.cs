    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
                ...
                if (!IsFreeShipping(restoredCart, _shippingSettings.FreeShippingOverXIncludingTax ? subTotalInclTax : subTotalExclTax))
                {
                    ...
                    if (!string.IsNullOrEmpty(updatedOrder.ShippingRateComputationMethodSystemName))
                    {
                        ...
                        if (updatedOrder.PickupInStore)
                        {
                            ...
                            {
                                var pickupPointsResponse = _shippingService.GetPickupPoints(updatedOrder.BillingAddress, user,
                    ...
                    else
                    {
                        ...
                        if (_shippingSettings.AllowPickupInStore)
                        {
                            ...
                            var pickupPointsResponse = _shippingService.GetPickupPoints(updatedOrder.BillingAddress, _workContext.CurrentUser, storeId: _storeContext.CurrentStore.Id);
                            if (pickupPointsResponse.Success)
                            {
                        ...
                        if (updateOrderParameters.PickupPoint == null)
                        {
                            ...
                            {
                                var userShippingAddress = _userService.GetUserShippingAddress(user);

                                var shippingOptionsResponse = _shippingService.GetShippingOptions(restoredCart, userShippingAddress, _workContext.CurrentUser, storeId: _storeContext.CurrentStore.Id);
                                if (shippingOptionsResponse.Success)
                                {
            ...
            bool? useRewardPoints, User user, decimal orderTotal)
        {
            ...
            if (!useRewardPoints.HasValue)
                useRewardPoints = _genericAttributeService.GetAttribute<bool>(user, WCoreUserDefaults.UseRewardPointsDuringCheckoutAttribute, _storeContext.CurrentStore.Id);
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            if (user != null)
            {
                var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(user, WCoreUserDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
            ...
            out decimal taxRate, out List<Discount> appliedDiscounts)
        {
            ...
            if (user != null)
                shippingOption = _genericAttributeService.GetAttribute<ShippingOption>(user, WCoreUserDefaults.SelectedShippingOptionAttribute, _storeContext.CurrentStore.Id);
            ...
            else
            {
                ...
                if (user != null)
                    shippingAddress = _userService.GetUserShippingAddress(user);
                ...
                if (!shippingRateComputationMethods.Any() && !_shippingSettings.AllowPickupInStore)
                    throw new WCoreException("Shipping rate computation method could not be loaded");
            ...
            bool? useRewardPoints = null, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            if (user != null)
            {
                paymentMethodSystemName = _genericAttributeService.GetAttribute<string>(user,
                    WCoreUserDefaults.SelectedPaymentMethodAttribute, _storeContext.CurrentStore.Id);
            }
