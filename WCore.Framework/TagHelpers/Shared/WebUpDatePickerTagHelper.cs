...
{
    /// <summary>
    /// WCore-date-picker tag helper
    /// </summary>
    [HtmlTargetElement("WCore-date-picker",
        Attributes = DayNameAttributeName + "," + MonthNameAttributeName + "," + YearNameAttributeName,
        TagStructure = TagStructure.WithoutEndTag)]
    public class WCoreDatePickerTagHelper : TagHelper
    {
        ...
        public WCoreDatePickerTagHelper(IHtmlGenerator generator, IHtmlHelper htmlHelper)
        {
            Generator = generator;
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            days.AppendFormat("<option value='{0}'>{1}</option>", "0", "GÜN");
            for (var i = 1; i <= 31; i++)
                days.AppendFormat("<option value='{0}'{1}>{0}</option>", i,
                    (SelectedDay.HasValue && SelectedDay.Value == i) ? " selected=\"selected\"" : null);

            months.AppendFormat("<option value='{0}'>{1}</option>", "0", "AY");
            for (var i = 1; i <= 12; i++)
            {
            ...
            years.AppendFormat("<option value='{0}'>{1}</option>", "0", "YIL");
