...
namespace WCore.Services.Catalog
{
    ...
    public partial class CopyProductService : ICopyProductService
    {
        ...
        protected virtual void CopyDiscountsMapping(Product product, Product productCopy)
        {
            ...
            {
                _productService.InsertDiscountAppliedToProduct(new DiscountAppliedToProduct { EntityId = productCopy.Id, DiscountId = discountMapping.DiscountId });
        ...
        protected virtual void CopyAssociatedProducts(Product product, bool isPublished, bool copyImages, bool copyAssociatedProducts, Product productCopy)
        {
            ...
            foreach (var associatedProduct in associatedProducts)
            {
                var associatedProductCopy = CopyProduct(associatedProduct,
                    string.Format(WCoreCatalogDefaults.ProductCopyNameTemplate, associatedProduct.Name),
        ...
        protected virtual void CopyTierPrices(Product product, Product productCopy)
        {
            foreach (var tierPrice in _productService.GetTierPricesByProduct(product.Id))
            {
                _productService.InsertTierPrice(new TierPrice
                {
                    ...
                    StartDateTime = tierPrice.StartDateTime,
                    EndDateTime = tierPrice.EndDateTime
                });
            }
        }
        ...
        protected virtual void CopyAttributesMapping(Product product, Product productCopy, Dictionary<int, int> originalNewPictureIdentifiers)
        {
            ...
            var oldCopyWithConditionAttributes = new List<ProductProductAttribute>();

            //all product attribute mapping copies
            var productProductAttributeCopies = new Dictionary<int, ProductProductAttribute>();

            var languages = _languageService.GetAllLanguages(true);

            foreach (var productProductAttribute in _productAttributeService.GetProductProductAttributesByProductId(product.Id))
            {
                var productProductAttributeCopy = new ProductProductAttribute
                {
                    ProductId = productCopy.Id,
                    ProductAttributeId = productProductAttribute.ProductAttributeId,
                    TextPrompt = productProductAttribute.TextPrompt,
                    IsRequired = productProductAttribute.IsRequired,
                    AttributeControlTypeId = productProductAttribute.AttributeControlTypeId,
                    DisplayOrder = productProductAttribute.DisplayOrder,
                    ValidationMinLength = productProductAttribute.ValidationMinLength,
                    ValidationMaxLength = productProductAttribute.ValidationMaxLength,
                    ValidationFileAllowedExtensions = productProductAttribute.ValidationFileAllowedExtensions,
                    ValidationFileMaximumSize = productProductAttribute.ValidationFileMaximumSize,
                    DefaultValue = productProductAttribute.DefaultValue
                };
                _productAttributeService.InsertProductProductAttribute(productProductAttributeCopy);
                //localization
                foreach (var lang in languages)
                {
                    var textPrompt = _localizationService.GetLocalized(productProductAttribute, x => x.TextPrompt, lang.Id, false, false);
                    if (!string.IsNullOrEmpty(textPrompt))
                        _localizedEntityService.SaveLocalizedValue(productProductAttributeCopy, x => x.TextPrompt, textPrompt,
                            lang.Id);
                }

                productProductAttributeCopies.Add(productProductAttributeCopy.Id, productProductAttributeCopy);

                if (!string.IsNullOrEmpty(productProductAttribute.ConditionAttributeXml))
                {
                    oldCopyWithConditionAttributes.Add(productProductAttribute);
                }

                //save associated value (used for combinations copying)
                associatedAttributes.Add(productProductAttribute.Id, productProductAttributeCopy.Id);

                // product attribute values
                var productAttributeValues = _productAttributeService.GetProductAttributeValues(productProductAttribute.Id);
                foreach (var productAttributeValue in productAttributeValues)
                {
                    ...
                    {
                        ProductProductAttributeId = productProductAttributeCopy.Id,
                        AttributeValueType = productAttributeValue.AttributeValueType,
            ...
            foreach (var productProductAttribute in oldCopyWithConditionAttributes)
            {
                var oldConditionAttributeMapping = _productAttributeParser
                    .ParseProductProductAttributes(productProductAttribute.ConditionAttributeXml).FirstOrDefault();

                if (oldConditionAttributeMapping == null)
                    continue;

                var oldConditionValues =
                    _productAttributeParser.ParseProductAttributeValues(productProductAttribute.ConditionAttributeXml,
                ...
                var newConditionAttributeMapping = productProductAttributeCopies[newAttributeMappingId];
                ...
                var attributeMappingId = associatedAttributes[productProductAttribute.Id];
                var conditionAttribute = productProductAttributeCopies[attributeMappingId];
                conditionAttribute.ConditionAttributeXml = newConditionAttributeXml;

                _productAttributeService.UpdateProductProductAttribute(conditionAttribute);
            }
            ...
            foreach (var combination in _productAttributeService.GetAllProductAttributeCombinations(product.Id))
            {
                ...
                var parsedProductAttributes = _productAttributeParser.ParseProductProductAttributes(combination.AttributesXml);
                foreach (var oldAttribute in parsedProductAttributes)
                {
                    if (!associatedAttributes.ContainsKey(oldAttribute.Id))
                        continue;

                    var newAttribute = _productAttributeService.GetProductProductAttributeById(associatedAttributes[oldAttribute.Id]);
        ...
        protected virtual Product CopyBaseProductData(Product product, string newName, bool isPublished)
        {
            ...
            var productCopy = new Product
            {
                ...
                PreOrderAvailabilityStartDateTime = product.PreOrderAvailabilityStartDateTime,
                ...
                MarkAsNewStartDateTime = product.MarkAsNewStartDateTime,
                MarkAsNewEndDateTime = product.MarkAsNewEndDateTime,
                Weight = product.Weight,
                Length = product.Length,
                Width = product.Width,
                Height = product.Height,
                AvailableStartDateTime = product.AvailableStartDateTime,
                AvailableEndDateTime = product.AvailableEndDateTime,
            ...
            bool isPublished = true, bool copyImages = true, bool copyAssociatedProducts = true)
        {
            ...
            {
                _productTagService.InsertProductProductTag(new ProductProductTag { ProductTagId = productTag.Id, ProductId = productCopy.Id });
            }
