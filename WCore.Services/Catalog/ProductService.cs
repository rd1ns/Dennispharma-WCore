...
namespace WCore.Services.Catalog
{
    ...
    /// Product service
    /// </summary>
    public partial class ProductService : Repository<Product>, IProductService
    {
        #region Fields
        ...
        protected readonly IRepository<DiscountAppliedToProduct> _discountProductMappingRepository;
        protected readonly IRepository<Product> _productRepository;
        protected readonly IRepository<ProductAttributeCombination> _productAttributeCombinationRepository;
        protected readonly IRepository<ProductProductAttribute> _productProductAttributeRepository;
        ...
        public ProductService(WCoreContext context, CatalogSettings catalogSettings,
            ...
            IRepository<DiscountAppliedToProduct> discountProductMappingRepository,
        ...
        protected virtual string GeStockMessage(Product product, string attributesXml)
        {
            ...
            else
            {
                ...
                {
                    stockMessage = !_productAttributeService.GetProductProductAttributesByProductId(product.Id)
        ...
        {
            var query = from p in context.Products.AsQueryable()
            ...
            var products = query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.ProductsAllDisplayedOnHomepageCacheKey));
        ...
        public virtual IList<Product> GetProductsByIds(int[] productIds)
        {
            ...
            var key = _cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.ProductsByIdsCacheKey, productIds);
        ...
        public virtual int GetNumberOfProductsInCategory(IList<int> categoryIds = null, int storeId = 0)
        {
            ...
            if (categoryIds != null && categoryIds.Any())
            {
                query = from p in query
                        join pc in context.ProductCategories.AsQueryable() on p.Id equals pc.ProductId
            ...
            {
                query = from p in query
                        join sm in context.StoreMappings.AsQueryable()
            ...
            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.CategoryNumberOfProductsCacheKey,
                allowedUserRolesIds, storeId, categoryIds);
            ...
            bool? overridePublished = null)
        {
            ...
            //Access control list. Allowed user roles
            var allowedUserRolesIds = 0;

            //pass category identifiers as comma-delimited string
            var commaSeparatedCategoryIds = categoryIds == null ? string.Empty : string.Join(",", categoryIds);

            //pass user role identifiers as comma-delimited string
            ...
            var products = context.Products.FromSqlRaw("ProductLoadAllPaged",
                pCategoryIds,
                pManufacturerId,
                pStoreId,
                pVendorId,
        ...
        {
            var query = from p in context.Products.AsQueryable()
                        join pam in context.Set<ProductProductAttribute>() on p.Id equals pam.ProductId
                        where
            ...
            int storeId = 0, int vendorId = 0, bool showHidden = false)
        {
            ...
            if (!showHidden)
            {
                ...
                //available dates
                query = query.Where(p =>
                    (!p.AvailableStartDateTime.HasValue || p.AvailableStartDateTime.Value < DateTime.Now) &&
                    (!p.AvailableEndDateTime.HasValue || p.AvailableEndDateTime.Value > DateTime.Now));
            }
            //vendor filtering
            if (vendorId > 0)
            {
        ...
        public virtual void UpdateProductReviewTotals(Product product)
        {
            ...
            var reviews = _productReviewRepository.GetAll().Where(r => r.ProductId == product.Id);
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            ...
            query = query.Where(product =>
                (product.UseMultipleWarehouses ? context.ProductWarehouseInventories.Where(pwi => pwi.ProductId == product.Id).Sum(pwi => pwi.StockQuantity - pwi.ReservedQuantity)
        ...
        public virtual IPagedList<ProductAttributeCombination> GetLowStockProductCombinations(int? vendorId = null, bool? loadPublishedOnly = true,
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            var combinations = from pac in context.ProductAttributeCombinations
                               join p in context.Products.AsQueryable() on pac.ProductId equals p.Id
                               where
        ...
        public virtual void UpdateHasDiscountsApplied(Product product)
        {
            ...
            product.HasDiscountsApplied = context.DiscountAppliedToProducts.Any(dpm => dpm.EntityId == product.Id);
            UpdateProduct(product);
        }
        ...
        public int GetNumberOfProductsByVendorId(int vendorId)
        {
            ...
            return context.Products.AsQueryable().Count(p => p.VendorId == vendorId && !p.Deleted);
        }
        ...
        public virtual int GetTotalStockQuantity(Product product, bool useReservedQuantity = true, int warehouseId = 0)
        {
            ...
            var pwi = context.ProductWarehouseInventories.Where(wi => wi.ProductId == product.Id);
        ...
        {
            return context.Products.AsQueryable().Any(p => productIds.Contains(p.Id) && p.IsDownload);
        }
        ...
        {
            return context.Products.AsQueryable().Any(p => productIds.Contains(p.Id) && p.IsGiftCard);
        }
        ...
        {
            return context.Products.AsQueryable().Any(p => productIds.Contains(p.Id) && p.IsRecurring);
        }
        ...
        public virtual void ReserveInventory(Product product, int quantity)
        {
            ...
            var productInventory = context.ProductWarehouseInventories.Where(pwi => pwi.ProductId == product.Id)
        ...
        public virtual void BalanceInventory(Product product, int warehouseId, int quantity)
        {
            ...
            var productInventory = context.ProductWarehouseInventories
            ...
            var productAnotherInventories = context.ProductWarehouseInventories
                .Where(pwi => pwi.ProductId == product.Id && pwi.WarehouseId != warehouseId)
        ...
        public virtual void BookReservedInventory(Product product, int warehouseId, int quantity, string message = "")
        {
            ...
            var pwi = context.ProductWarehouseInventories.FirstOrDefault(wi => wi.ProductId == product.Id && wi.WarehouseId == warehouseId);
            if (pwi == null)
                return;
        ...
        public virtual int ReverseBookedInventory(Product product, ShipmentItem shipmentItem, string message = "")
        {
            ...
            var pwi = context.ProductWarehouseInventories.FirstOrDefault(wi => wi.ProductId == product.Id && wi.WarehouseId == shipmentItem.WarehouseId);
            if (pwi == null)
                return 0;
        ...
        public virtual IList<RelatedProduct> GetRelatedProductsByProductId1(int productId, bool showHidden = false)
        {
            var query = from rp in context.RelatedProducts
                        join p in context.Products.AsQueryable() on rp.ProductId2 equals p.Id
            ...
            var relatedProducts = query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.ProductsRelatedCacheKey, productId, showHidden));
        ...
        public virtual IList<CrossSellProduct> GetCrossSellProductsByProductIds(int[] productIds, bool showHidden = false)
        {
            ...
            var query = from csp in context.CrossSellProducts
                        join p in context.Products.AsQueryable() on csp.ProductId2 equals p.Id
        ...
        public virtual IList<TierPrice> GetTierPrices(Product product, User user, int storeId)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
        ...
        {
            return context.TierPrices.Where(tp => tp.ProductId == productId)
                .ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.ProductTierPricesCacheKey, productId));
        }
        ...
        /// <param name="quantity">Quantity</param>
        /// <returns>Tier price</returns>
        public virtual TierPrice GetPreferredTierPrice(Product product, User user, int storeId, int quantity)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
            ...
            return GetTierPrices(product, user, storeId)?.LastOrDefault(price => quantity >= price.Quantity);
        }
        ...
        {
            var productPictures = context.ProductPictures.Where(p => productsIds.Contains(p.ProductId)).ToList();
        ...
        {
            var products = context.Products.AsQueryable().Where(product => product.HasDiscountsApplied);

            if (discountId.HasValue)
                products = from product in products
                           join dpm in context.DiscountAppliedToProducts on product.Id equals dpm.EntityId
        ...
        /// <param name="from">Item creation from; null to load all records</param>
        /// <param name="to">Item item creation to; null to load all records</param>
        ...
        public virtual IPagedList<ProductReview> GetAllProductReviews(int userId = 0, bool? approved = null,
            DateTime? from = null, DateTime? to = null,
            string message = null, int storeId = 0, int productId = 0, int vendorId = 0, bool showHidden = false,
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _productReviewRepository.GetAll().AsQueryable();
            ...
            query = from productReview in query
                    join product in context.Products.AsQueryable() on productReview.ProductId equals product.Id
                    where
            ...
            if (storeId > 0 && !showHidden && !_catalogSettings.IgnoreStoreLimitations)
            {
                query = from productReview in query
                        join product in context.Products.AsQueryable() on productReview.ProductId equals product.Id
                        join storeMapping in context.StoreMappings.AsQueryable()
            ...
            query = _catalogSettings.ProductReviewsSortByCreatedDateAscending
                ? query.OrderBy(pr => pr.CreatedOn).ThenBy(pr => pr.Id)
                : query.OrderByDescending(pr => pr.CreatedOn).ThenBy(pr => pr.Id);
        ...
        public virtual void DeleteProductReviews(IList<ProductReview> productReviews)
        {
            ...
            _productReviewRepository.BulkDelete(productReviews);
        }
        ...
        public virtual void SetProductReviewHelpfulness(ProductReview productReview, bool helpfulness)
        {
            ...
            var prh = context.ProductReviewHelpfulnesses.SingleOrDefault(h => h.ProductReviewId == productReview.Id && h.UserId == _workContext.CurrentUser.Id);
        ...
        public virtual (int usefulCount, int notUsefulCount) GetHelpfulnessCounts(ProductReview productReview)
        {
            ...
            var productReviewHelpfulness = context.ProductReviewHelpfulnesses.Where(prh => prh.ProductReviewId == productReview.Id);
        ...
        {
            return context.ProductWarehouseInventories.Where(pwi => pwi.ProductId == productId).ToList();
        }
        ...
        {
            return (from w in context.Warehouses
                    join pwi in context.ProductWarehouseInventories on w.Id equals pwi.WarehouseId
        ...
        public virtual void UpdateProductWarehouseInventory(IList<ProductWarehouseInventory> pwis)
        {
            ...
            _productWarehouseInventoryRepository.BulkUpdate(pwis);
        }
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            var query = context.StockQuantityHistories.Where(historyEntry => historyEntry.ProductId == product.Id);
            ...
            query = query.OrderByDescending(historyEntry => historyEntry.CreatedOn).ThenByDescending(historyEntry => historyEntry.Id);
        ...
        public virtual void ClearDiscountAppliedToProduct(Discount discount)
        {
            if (discount is null)
                throw new ArgumentNullException(nameof(discount));

            var mappingsWithProducts =
                from dcm in context.DiscountAppliedToProducts
                join p in context.Products.AsQueryable() on dcm.EntityId equals p.Id
        ...
        /// </summary>
        /// <param name="productId">Product identifier</param>
        public virtual IList<DiscountAppliedToProduct> GetAllDiscountsAppliedToProduct(int productId)
        {
            return context.DiscountAppliedToProducts.Where(dcm => dcm.EntityId == productId).ToList();
        }
        ...
        /// <returns>Result</returns>
        public virtual DiscountAppliedToProduct GetDiscountAppliedToProduct(int productId, int discountId)
        {
            return context.DiscountAppliedToProducts.FirstOrDefault(dcm => dcm.EntityId == productId && dcm.DiscountId == discountId);
        }
        ...
        public virtual void InsertDiscountAppliedToProduct(DiscountAppliedToProduct discountProductMapping)
        {
        ...
        /// </summary>
        /// <param name="discountProductMapping">Discount-product mapping</param>
        public virtual void DeleteDiscountAppliedToProduct(DiscountAppliedToProduct discountProductMapping)
        {
            if (discountProductMapping is null)
                throw new ArgumentNullException(nameof(discountProductMapping));
