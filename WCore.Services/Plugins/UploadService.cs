...
namespace WCore.Services.Plugins
{
    ...
    public class UploadService : IUploadService
    {
        ...
        private readonly IWCoreFileProvider _fileProvider;
        ...
        public UploadService(IWCoreFileProvider fileProvider,
        ...
        protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
        {
            ...
            var uploadedItemsFileEntry = archive.Entries
                .FirstOrDefault(entry => entry.Name.Equals(WCorePluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
        ...
        protected virtual IDescriptor UploadSingleItem(string archivePath)
        {
            ...
            var pluginsDirectory = _fileProvider.MapPath(WCorePluginDefaults.Path);

            //get path to the themes directory
            var themesDirectory = string.Empty;
            if (!string.IsNullOrEmpty(WCorePluginDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(WCorePluginDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception("The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{WCorePluginDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{WCorePluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{WCorePluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
                    ...
                    if (isPluginDescriptor)
                    {
                        ...
                        if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(WCoreVersion.CurrentVersion))
                            throw new Exception($"This plugin doesn't support the current version - {WCoreVersion.CurrentVersion}");
                    }
        ...
        protected virtual IList<IDescriptor> UploadMultipleItems(string archivePath, IList<UploadedItem> uploadedItems)
        {
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    //ensure that the current version of wCoreCommerce is supported
                    if (!item.SupportedVersions?.Contains(WCoreVersion.CurrentVersion) ?? true)
                        continue;
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{WCorePluginDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(WCorePluginDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{WCorePluginDefaults.ThemeDescriptionFileName}";
                    ...
                    //ensure that the plugin current version is supported
                    if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(WCoreVersion.CurrentVersion))
                        continue;
        ...
        public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(WCorePluginDefaults.UploadsTempPath);
                ...
                //you can find a sample of such descriptive file in Libraries\WCore.Core\Plugins\Samples\
        ...
        public virtual void UploadIconsArchive(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var storeIconsPath = _fileProvider.GetAbsolutePath(string.Format(WCoreCommonDefaults.FaviconAndAppIconsPath, _storeContext.ActiveStoreScopeConfiguration));
        ...
        public virtual void UploadFavicon(IFormFile favicon)
        {
            ...
            var storeFaviconPath = _fileProvider.GetAbsolutePath(string.Format(WCoreCommonDefaults.FaviconAndAppIconsPath, _storeContext.ActiveStoreScopeConfiguration));
        ...
        public virtual void UploadLocalePattern()
        {
            ...
            //    var ziplocalePatternPath = getPath(WCoreCommonDefaults.LocalePatternPath, WCoreCommonDefaults.LocalePatternArchiveName);
            //    if (!_fileProvider.GetFileExtension(ziplocalePatternPath)?.Equals(".zip", StringComparison.InvariantCultureIgnoreCase) ?? true)
            //        throw new Exception($"Archive '{WCoreCommonDefaults.LocalePatternArchiveName}' to retrieve localization patterns not found.");
            ...
            //    if (!(checkDirectoryExists(WCoreCommonDefaults.LocalePatternPath, currentCulture.Name) || 
            //        checkDirectoryExists(WCoreCommonDefaults.LocalePatternPath, currentCulture.TwoLetterISOLanguageName)))
            //    {
            //        var cultureToUse = string.Empty;

            //        //3. Unpack the archive into a temporary folder
            //        ZipFile.ExtractToDirectory(ziplocalePatternPath, getPath(WCoreCommonDefaults.LocalePatternPath, tempFolder));

            //        //4. Search in the temp unpacked archive a folder with locales by culture
            //        var sourcelocalePath = _fileProvider.Combine(getPath(WCoreCommonDefaults.LocalePatternPath, tempFolder), currentCulture.Name);
            //        if (_fileProvider.DirectoryExists(sourcelocalePath))
            //        {
            //            cultureToUse = currentCulture.Name;
            //        }

            //        var sourcelocaleISOPath = _fileProvider.Combine(getPath(WCoreCommonDefaults.LocalePatternPath, tempFolder), currentCulture.TwoLetterISOLanguageName);
            ...
            //        {
            //            var destlocalePath = getPath(WCoreCommonDefaults.LocalePatternPath, cultureToUse);
            ...
            //    if (checkDirectoryExists(WCoreCommonDefaults.LocalePatternPath, tempFolder))
            //        _fileProvider.DeleteDirectory(getPath(WCoreCommonDefaults.LocalePatternPath, tempFolder));
            //}
        }
        ...
        public class UploadedItem
        {
            ...
            /// <summary>
            /// Gets or sets supported versions of wCoreCommerce
            /// </summary>
