    ...
    public static class HtmlExtensions
    {
            ...
            where TLocalizedModelLocal : ILocalizedLocaleModel
        {
            ...
            if (localizationSupported)
            {
                ...
                tabStrip.AppendLine("<ul class=\"nav nav-tabs nav-bold nav-tabs-line nav-tabs-line-3x mb-5\">");
                ...
                tabStrip.AppendLine(string.Format("<li{0}>", standardTabSelected ? " class=\"nav-item mr-3 active\"" : " class=\"nav-item mr-3\""));
                tabStrip.AppendLine($"<a class=\"nav-link {(standardTabSelected ? "active" : "")} \" data-tab-name=\"{standardTabName}\" href=\"#{standardTabName}\" data-toggle=\"tab\">{EngineContext.Current.Resolve<ILocalizationService>().GetResource("Admin.Common.Standard")}</a>");
                ...
                {
                    //languages
                    var language = languageService.GetById(locale.LanguageId, cache => default);
                    if (language == null)
                        throw new Exception("Language cannot be loaded");

                    var localizedTabName = $"{name}-{language.Id}-tab";
                    tabStrip.AppendLine(string.Format("<li{0}>", localizedTabName == tabNameToSelect ? " class=\"nav-item mr-3\"" : " class=\"nav-item mr-3\""));
                    var iconUrl = urlHelper.Content("~/images/flags/" + language.FlagImageFileName);
                    tabStrip.AppendLine($"<a class=\"nav-link {(standardTabSelected ? "" : "")} \" data-tab-name=\"{localizedTabName}\" href=\"#{localizedTabName}\" data-toggle=\"tab\"><img alt='' src='{iconUrl}' class=\"pr-2\">{WebUtility.HtmlEncode(language.Name)}</a>");
                ...
                tabStrip.AppendLine(string.Format("<div role=\"tabpanel\" class=\"tab-pane{0} px-7\" id=\"{1}\">", standardTabSelected ? " show active" : null, standardTabName));
                ...
                {
                    //languages
                    var language = languageService.GetById(helper.ViewData.Model.Locales[i].LanguageId, cache => default);
                    if (language == null)
                        throw new Exception("Language cannot be loaded");

                    var localizedTabName = $"{name}-{language.Id}-tab";
                    tabStrip.AppendLine(string.Format("<div role=\"tabpanel\"  class=\"tab-pane{0} px-7\" id=\"{1}\">", localizedTabName == tabNameToSelect ? " active" : null, localizedTabName));
        ...
        public static string GetSelectedPanelName(this IHtmlHelper helper)
        {
            ...
            const string dataKey = "WCore.selected-panel-name";
        ...
        public static string GetSelectedTabName(this IHtmlHelper helper, string dataKeyPrefix = null)
        {
            ...
            var tabName = string.Empty;
            var dataKey = "WCore.selected-tab-name";
        ...
        /// <summary>
        /// Gets a selected accordion name (used in admin area to store selected accordion name)
        /// </summary>
        /// <param name="helper">HtmlHelper</param>
        /// <param name="dataKeyPrefix">Key prefix. Pass null to ignore</param>
        /// <returns>Name</returns>
        public static string GetSelectedAccordionName(this IHtmlHelper helper, string dataKeyPrefix = null)
        {
            //keep this method synchronized with
            //"SaveSelectedAccordion" method of \Area\Admin\Controllers\BaseAdminController.cs
            var accordionName = string.Empty;
            var dataKey = "WCore.selected-accordion-name";
            if (!string.IsNullOrEmpty(dataKeyPrefix))
                dataKey += $"-{dataKeyPrefix}";

            if (helper.ViewData.ContainsKey(dataKey))
                accordionName = helper.ViewData[dataKey].ToString();

            if (helper.ViewContext.TempData.ContainsKey(dataKey))
                accordionName = helper.ViewContext.TempData[dataKey].ToString();
