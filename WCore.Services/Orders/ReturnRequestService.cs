...
namespace WCore.Services.Orders
{
    ...
    public partial class ReturnRequestService : IReturnRequestService
    {
        ...
        {
            var query = _returnRequestRepository.GetAll().AsQueryable();
            if (storeId > 0)
            ...
            if (createdFrom.HasValue)
                query = query.Where(rr => createdFrom.Value <= rr.CreatedOn);
            if (createdTo.HasValue)
                query = query.Where(rr => createdTo.Value >= rr.CreatedOn);

            query = query.OrderByDescending(rr => rr.CreatedOn).ThenByDescending(rr => rr.Id);
        ...
        {
            var query = from rra in _returnRequestActionRepository.GetAll().AsQueryable()
                        orderby rra.DisplayOrder, rra.Id
                        select rra;

            return query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreOrderDefaults.ReturnRequestActionAllCacheKey));
        }
        ...
        {
            var query = from rra in _returnRequestReasonRepository.GetAll().AsQueryable()
                        orderby rra.DisplayOrder, rra.Id
                        select rra;

            return query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreOrderDefaults.ReturnRequestReasonAllCacheKey));
        }
