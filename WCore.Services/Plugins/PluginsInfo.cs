...
namespace WCore.Services.Plugins
{
    ...
    public partial class PluginsInfo : IPluginsInfo
    {
        ...
        protected virtual IList<string> GetObsoleteInstalledPluginNames()
        {
            ...
            var filePath = _fileProvider.MapPath(WCorePluginDefaults.InstalledPluginsFilePath);
            if (!_fileProvider.FileExists(filePath))
            {
                //if not, try to parse the file that was used in previous wCoreCommerce versions
                filePath = _fileProvider.MapPath(WCorePluginDefaults.ObsoleteInstalledPluginsFilePath);
                if (!_fileProvider.FileExists(filePath))
        ...
        public PluginsInfo(IWCoreFileProvider fileProvider)
        {
        ...
        {
            //save the file
            var filePath = _fileProvider.MapPath(WCorePluginDefaults.PluginsInfoFilePath);
