...
namespace WCore.Services.Orders
{
    ...
    public partial class OrderReportService : IOrderReportService
    {
        ...
        public virtual IList<OrderByCountryReportLine> GetCountryReport(int storeId, OrderStatus? os,
            PaymentStatus? ps, ShippingStatus? ss, DateTime? startTime, DateTime? endTime)
        {
            ...
            if (startTime.HasValue)
                query = query.Where(o => startTime.Value <= o.CreatedOn);
            if (endTime.HasValue)
                query = query.Where(o => endTime.Value >= o.CreatedOn);

            var report = (from oq in query
                          join a in _addressRepository.GetAll().AsQueryable() on oq.BillingAddressId equals a.Id
            ...
            string billingPhone = null, string billingEmail = null, string billingLastName = "", string orderNotes = null)
        {
            ...
            if (vendorId > 0)
                query = from o in query
                    join oi in _orderItemRepository.GetAll().AsQueryable() on o.Id equals oi.OrderId
                    join p in _productRepository.GetAll().AsQueryable() on oi.ProductId equals p.Id
            ...
            if (warehouseId > 0)
            {
                ...
                query = from o in query
                    join oi in _orderItemRepository.GetAll().AsQueryable() on o.Id equals oi.OrderId
                    join p in _productRepository.GetAll().AsQueryable() on oi.ProductId equals p.Id
                    join pwi in _productWarehouseInventoryRepository.GetAll().AsQueryable() on p.Id equals pwi.ProductId
                    where
            ...
            query = from o in query
                join oba in _addressRepository.GetAll().AsQueryable() on o.BillingAddressId equals oba.Id
                where 
            ...
            if (!string.IsNullOrEmpty(orderNotes))
                query = from o in query
                    join n in _orderNoteRepository.GetAll().AsQueryable() on o.Id equals n.OrderId
        ...
        public virtual OrderAverageReportLineSummary OrderAverageReport(int storeId, OrderStatus os)
        {
            ...
            DateTime? startTime1 = _dateTimeHelper.ConvertToTime(t1, timeZone);
            ...
            DateTime? startTime2 = _dateTimeHelper.ConvertToTime(t2, timeZone);
            ...
            DateTime? startTime3 = _dateTimeHelper.ConvertToTime(t3, timeZone);
            ...
            DateTime? startTime4 = _dateTimeHelper.ConvertToTime(t4, timeZone);
        ...
        /// <param name="createdFrom">Order created date from (UTC); null to load all records</param>
        /// <param name="createdTo">Order created date to (UTC); null to load all records</param>
            ...
            bool showHidden = false)
        {
            ...
            var query1 = from orderItem in _orderItemRepository.GetAll().AsQueryable()
                         join o in _orderRepository.GetAll().AsQueryable() on orderItem.OrderId equals o.Id
                         join p in _productRepository.GetAll().AsQueryable() on orderItem.ProductId equals p.Id
                         join oba in _addressRepository.GetAll().AsQueryable() on o.BillingAddressId equals oba.Id
                         join pc in _productCategoryRepository.GetAll().AsQueryable() on p.Id 
                             equals pc.ProductId 
                             into p_pc
                from pc in p_pc.DefaultIfEmpty()
                         join pm in _productManufacturerRepository.GetAll().AsQueryable() on p.Id 
                             equals pm.ProductId 
                             into p_pm
                from pm in p_pm.DefaultIfEmpty()
                         where (storeId == 0 || storeId == o.StoreId) &&
                               (!createdFrom.HasValue || createdFrom.Value <= o.CreatedOn) &&
                               (!createdTo.HasValue || createdTo.Value >= o.CreatedOn) &&
            ...
            int recordsToReturn = 5, bool visibleIndividuallyOnly = true, bool showHidden = false)
        {
            ...
            var query2 = from orderItem in _orderItemRepository.GetAll().AsQueryable()
                         join p in _productRepository.GetAll().AsQueryable() on orderItem.ProductId equals p.Id
                         join o in _orderRepository.GetAll().AsQueryable() on orderItem.OrderId equals o.Id
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            var query_tmp = (from orderItem in _orderItemRepository.GetAll().AsQueryable()
                             join o in _orderRepository.GetAll().AsQueryable() on orderItem.OrderId equals o.Id
                             where (!createdFrom.HasValue || createdFrom.Value <= o.CreatedOn) &&
                                   (!createdTo.HasValue || createdTo.Value >= o.CreatedOn) &&
                                   !o.Deleted
            ...
            var query = from p in _productRepository.GetAll().AsQueryable()
                        join pm in _productManufacturerRepository.GetAll().AsQueryable() on p.Id 
                            equals pm.ProductId 
                            into p_pm
                from pm in p_pm.DefaultIfEmpty()
            ...
            string billingPhone = null, string billingEmail = null, string billingLastName = "", string orderNotes = null)
        {
            ...
            var orders = _orderRepository.GetAll().AsQueryable();
            if (osIds != null && osIds.Any())
            ...
            var query = from orderItem in _orderItemRepository.GetAll().AsQueryable()
                ...
                where (storeId == 0 || storeId == o.StoreId) &&
                      ...
                      //    _productWarehouseInventoryRepository.GetAll().Any(pwi =>
                      ...
                      //(dontSearchOrderNotes || _orderNoteRepository.GetAll().Any(oNote =>
