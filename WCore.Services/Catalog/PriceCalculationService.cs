...
namespace WCore.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
        ...
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToProduct(Product product, User user)
        {
        ...
        /// <param name="user">User</param>
        /// <returns>Discounts</returns>
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToCategories(Product product, User user)
        {
            var allowedDiscounts = new List<Discount>();
            if (_catalogSettings.IgnoreDiscounts)
                return allowedDiscounts;
            ...
            foreach (var discount in _discountService.GetAllDiscounts(DiscountType.AssignedToCategories))
            {
                ...
                var discountCategoryIds = _categoryService.GetAppliedCategoryIds(discount, user);
        ...
        /// <param name="user">User</param>
        /// <returns>Discounts</returns>
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToManufacturers(Product product, User user)
        {
            var allowedDiscounts = new List<Discount>();
            if (_catalogSettings.IgnoreDiscounts)
                return allowedDiscounts;
            ...
            foreach (var discount in _discountService.GetAllDiscounts(DiscountType.AssignedToManufacturers))
            {
                ...
                var discountManufacturerIds = _manufacturerService.GetAppliedManufacturerIds(discount, user);
        ...
        /// <param name="user">User</param>
        /// <returns>Discounts</returns>
        protected virtual IList<Discount> GetAllowedDiscounts(Product product, User user)
        {
            var allowedDiscounts = new List<Discount>();
            if (_catalogSettings.IgnoreDiscounts)
                return allowedDiscounts;
            ...
            out List<Discount> appliedDiscounts)
        {
            ...
            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.ProductPriceCacheKey, 
                product,
        ...
        public virtual decimal GetProductAttributeValuePriceAdjustment(Product product, ProductAttributeValue value, User user, decimal? productPrice = null)
        {
            if (value == null)
                ...
                case AttributeValueType.Simple:
                    ...
                    if (value.PriceAdjustmentUsePercentage)
                    {
                        if (!productPrice.HasValue)
                            productPrice = GetFinalPrice(product, user);
        ...
        public virtual decimal RoundPrice(decimal value, Currency currency = null)
        {
            ...
            currency ??= _currencyService.GetById(_currencySettings.PrimaryStoreCurrencyId);
