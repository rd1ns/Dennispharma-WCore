    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual PlaceOrderContainer PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (_userService.IsGuest(details.User) && !_orderSettings.AnonymousCheckoutAllowed)
                throw new WCoreException("Anonymous checkout is not allowed");

            //user currency
            var currencyTmp = _currencyService.GetById(
                _genericAttributeService.GetAttribute<int>(details.User, WCoreUserDefaults.CurrencyIdAttribute, processPaymentRequest.StoreId));
            ...
            //user language
            details.UserLanguage = _languageService.GetById(
                _genericAttributeService.GetAttribute<int>(details.User, WCoreUserDefaults.LanguageIdAttribute, processPaymentRequest.StoreId));
            ...
            if (details.User.BillingAddressId is null)
                throw new WCoreException("Billing address is not provided");

            var billingAddress = _userService.GetUserBillingAddress(details.User);

            if (!CommonHelper.IsValidEmail(billingAddress?.Email))
                throw new WCoreException("Email is not valid");
            ...
            if (_countryService.GetCountryByAddress(details.BillingAddress) is Country billingCountry && !billingCountry.AllowsBilling)
                throw new WCoreException($"Country '{billingCountry.Name}' is not allowed for billing");

            //checkout attributes
            details.CheckoutAttributesXml = _genericAttributeService.GetAttribute<string>(details.User, WCoreUserDefaults.CheckoutAttributes, processPaymentRequest.StoreId);
            ...
            //load shopping cart
            details.Cart = _shoppingCartService.GetShoppingCart(details.User, ShoppingCartType.ShoppingCart, processPaymentRequest.StoreId);

            if (!details.Cart.Any())
                throw new WCoreException("Cart is empty");

            //validate the entire shopping cart
            var warnings = _shoppingCartService.GetShoppingCartWarnings(details.Cart, details.CheckoutAttributesXml, true);
            if (warnings.Any())
                throw new WCoreException(warnings.Aggregate(string.Empty, (current, next) => $"{current}{next};"));
            ...
            foreach (var sci in details.Cart)
            {
                ...
                var sciWarnings = _shoppingCartService.GetShoppingCartItemWarnings(details.User,
                    ...
                    sci.UserEnteredPrice, sci.RentalStartDate, sci.RentalEndDate, sci.Quantity, false, sci.Id);
                if (sciWarnings.Any())
                    throw new WCoreException(sciWarnings.Aggregate(string.Empty, (current, next) => $"{current}{next};"));
            }
            ...
            if (!ValidateMinOrderSubtotalAmount(details.Cart))
            {
                ...
                throw new WCoreException(string.Format(_localizationService.GetResource("Checkout.MinOrderSubtotalAmount"),
            ...
            if (!ValidateMinOrderTotalAmount(details.Cart))
            {
                ...
                throw new WCoreException(string.Format(_localizationService.GetResource("Checkout.MinOrderTotalAmount"),
                    _priceFormatter.FormatPrice(minOrderTotalAmount, true, false)));
            }

            //tax display type
            if (_taxSettings.AllowUsersToSelectTaxDisplayType)
                details.UserTaxDisplayType = (TaxDisplayType)_genericAttributeService.GetAttribute<int>(details.User, WCoreUserDefaults.TaxDisplayTypeIdAttribute, processPaymentRequest.StoreId);
            else
            ...
            if (_shoppingCartService.ShoppingCartRequiresShipping(details.Cart))
            {
                var pickupPoint = _genericAttributeService.GetAttribute<PickupPoint>(details.User,
                    WCoreUserDefaults.SelectedPickupPointAttribute, processPaymentRequest.StoreId);
                ...
                else
                {
                    if (details.User.ShippingAddressId == null)
                        throw new WCoreException("Shipping address is not provided");

                    var shippingAddress = _userService.GetUserShippingAddress(details.User);
                    ...
                    if (_countryService.GetCountryByAddress(details.ShippingAddress) is Country shippingCountry && !shippingCountry.AllowsShipping)
                        throw new WCoreException($"Country '{shippingCountry.Name}' is not allowed for shipping");
                }

                var shippingOption = _genericAttributeService.GetAttribute<ShippingOption>(details.User,
                    WCoreUserDefaults.SelectedShippingOptionAttribute, processPaymentRequest.StoreId);
            ...
            if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                throw new WCoreException("Shipping total couldn't be calculated");
            ...
            //VAT number
            var userVatStatus = (VatNumberStatus)_genericAttributeService.GetAttribute<int>(details.User, WCoreUserDefaults.VatNumberStatusIdAttribute);
            if (_taxSettings.EuVatEnabled && userVatStatus == VatNumberStatus.Valid)
                details.VatNumber = _genericAttributeService.GetAttribute<string>(details.User, WCoreUserDefaults.VatNumberAttribute);
            ...
            if (!orderTotal.HasValue)
                throw new WCoreException("Order total couldn't be calculated");
            ...
            if (!string.IsNullOrEmpty(recurringCyclesError))
                throw new WCoreException(recurringCyclesError);
        ...
        protected virtual PlaceOrderContainer PrepareRecurringOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            //user language
            details.UserLanguage = _languageService.GetById(details.InitialOrder.UserLanguageId);
            if (details.UserLanguage == null || !details.UserLanguage.Published)
                details.UserLanguage = _workContext.WorkingLanguage;

            //billing address
            if (details.InitialOrder.BillingAddressId == 0)
                throw new WCoreException("Billing address is not available");
            ...
            if (details.InitialOrder.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                ...
                if (!details.PickupInStore)
                {
                    if (!details.InitialOrder.ShippingAddressId.HasValue || !(_addressService.GetAddressById(details.InitialOrder.ShippingAddressId.Value) is Address shippingAddress))
                        throw new WCoreException("Shipping address is not available");
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            ...
            _userService.Update(details.User);
        ...
        protected virtual void AwardRewardPoints(Order order)
        {
            ...
            if (_rewardPointsSettings.ActivationDelay > 0)
            {
                ...
                activatingDate = DateTime.Now.AddHours(delayInHours);
            }

            //whether points validity is set
            DateTime? endDate = null;
            if (_rewardPointsSettings.PurchasesPointsValidity > 0)
                endDate = (activatingDate ?? DateTime.Now).AddDays(_rewardPointsSettings.PurchasesPointsValidity.Value);
        ...
        protected virtual void ReduceRewardPoints(Order order)
        {
            ...
            if (rewardPointsHistoryEntry != null && rewardPointsHistoryEntry.CreatedOn > DateTime.Now)
            {
        ...
        protected virtual void ReturnBackRedeemedRewardPoints(Order order)
        {
            ...
            foreach (var rewardPoints in _rewardPointService.GetRewardPointsHistory(order.UserId, order.StoreId, orderGuid: order.OrderGuid))
                //return back
                _rewardPointService.AddRewardPointsHistoryEntry(user, -rewardPoints.Points, order.StoreId,
            ...
            foreach (var gc in giftCards)
            {
                if (activate)
                {
                    ...
                    if (gc.GiftCardType == GiftCardType.Virtual)
                    {
                        ...
                        {
                            var userLang = _languageService.GetById(order.UserLanguageId) ??
        ...
        protected virtual void ProcessOrderPaid(Order order)
        {
            ...
            if (order.OrderTotal != decimal.Zero)
            {
                ...
                var orderPaidAttachmentFileName = _orderSettings.AttachPdfInvoiceToOrderPaidEmail ?
                    "order.pdf" : null;
                var orderPaidUserNotificationQueuedEmailIds = _workflowMessageService.SendOrderPaidUserNotification(order, order.UserLanguageId,
                    orderPaidAttachmentFilePath, orderPaidAttachmentFileName);

                if (orderPaidUserNotificationQueuedEmailIds.Any())
                    AddOrderNote(order, $"\"Order paid\" email (to user) has been queued. Queued email identifiers: {string.Join(", ", orderPaidUserNotificationQueuedEmailIds)}.");
        ...
        protected virtual void MoveShoppingCartItemsToOrderItems(PlaceOrderContainer details, Order order)
        {
            foreach (var sc in details.Cart)
            {
                ...
                var orderItem = new OrderItem
                {
                    ...
                    RentalStartDate = sc.RentalStartDate,
                    RentalEndDate = sc.RentalEndDate
                };
        ...
        protected virtual ProcessPaymentResult GetProcessPaymentResult(ProcessPaymentRequest processPaymentRequest, PlaceOrderContainer details)
        {
            ...
            if (!skipPaymentWorkflow)
            {
                ...
                var paymentMethod = _paymentPluginManager
                    .LoadPluginBySystemName(processPaymentRequest.PaymentMethodSystemName, user, processPaymentRequest.StoreId)
                    ?? throw new WCoreException("Payment method couldn't be loaded");

                //ensure that payment method is active
                if (!_paymentPluginManager.IsPluginActive(paymentMethod))
                    throw new WCoreException("Payment method is not active");
                ...
                if (details.IsRecurringShoppingCart)
                {
                    ...
                    {
                        case RecurringPaymentType.NotSupported:
                            throw new WCoreException("Recurring payments are not supported by selected payment method");
                        case RecurringPaymentType.Manual:
                        case RecurringPaymentType.Automatic:
                            processPaymentResult = _paymentService.ProcessRecurringPayment(processPaymentRequest);
                            break;
                        default:
                            throw new WCoreException("Not supported recurring payment type");
                    }
                }
                else
                    //standard cart
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (processPaymentResult == null)
                    throw new WCoreException("processPaymentResult is not available");
        ...
        public virtual void UpdateOrderTotals(UpdateOrderParameters updateOrderParameters)
        {
            ...
            if (!itemDeleted)
            {
                ...
                updateOrderParameters.Warnings.AddRange(_shoppingCartService.GetShoppingCartItemWarnings(user, updatedShoppingCartItem.ShoppingCartType,
                    ...
                    updatedShoppingCartItem.RentalStartDate, updatedShoppingCartItem.RentalEndDate, updatedShoppingCartItem.Quantity, false, updatedShoppingCartItem.Id));
        ...
        public virtual IEnumerable<string> ProcessNextRecurringPayment(RecurringPayment recurringPayment, ProcessPaymentResult paymentResult = null)
        {
            ...
            try
            {
                if (!recurringPayment.IsActive)
                    throw new WCoreException("Recurring payment is not active");

                var initialOrder = _orderService.GetOrderById(recurringPayment.InitialOrderId);
                if (initialOrder == null)
                    throw new WCoreException("Initial order could not be loaded");

                var user = _userService.GetUserById(initialOrder.UserId);
                if (user == null)
                    throw new WCoreException("User could not be loaded");

                if (GetNextPaymentDate(recurringPayment) is null)
                    throw new WCoreException("Next payment date could not be calculated");
                ...
                {
                    var paymentMethod = _paymentPluginManager
                        .LoadPluginBySystemName(processPaymentRequest.PaymentMethodSystemName, user, initialOrder.StoreId)
                        ?? throw new WCoreException("Payment method couldn't be loaded");
                    ...
                    {
                        RecurringPaymentType.NotSupported => throw new WCoreException("Recurring payments are not supported by selected payment method"),
                        ...
                        _ => throw new WCoreException("Not supported recurring payment type"),
                    };
                }
                else
        ...
        public virtual void CancelOrder(Order order, bool notifyUser)
        {
            ...
            if (!CanCancelOrder(order))
                throw new WCoreException("Cannot do cancel for order.");
        ...
        public virtual IList<string> Capture(Order order)
        {
            ...
            if (!CanCapture(order))
                throw new WCoreException("Cannot do capture for order.");
        ...
        public virtual void MarkOrderAsPaid(Order order)
        {
            ...
            if (!CanMarkOrderAsPaid(order))
                throw new WCoreException("You can't mark this order as paid");
        ...
        public virtual IList<string> Refund(Order order)
        {
            ...
            if (!CanRefund(order))
                throw new WCoreException("Cannot do refund for order.");
        ...
        public virtual void RefundOffline(Order order)
        {
            ...
            if (!CanRefundOffline(order))
                throw new WCoreException("You can't refund this order");
        ...
        public virtual IList<string> PartiallyRefund(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefund(order, amountToRefund))
                throw new WCoreException("Cannot do partial refund for order.");
        ...
        public virtual void PartiallyRefundOffline(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefundOffline(order, amountToRefund))
                throw new WCoreException("You can't partially refund (offline) this order");
        ...
        public virtual IList<string> Void(Order order)
        {
            ...
            if (!CanVoid(order))
                throw new WCoreException("Cannot do void for order.");
        ...
        public virtual void VoidOffline(Order order)
        {
            ...
            if (!CanVoidOffline(order))
                throw new WCoreException("You can't void this order");
        ...
        public virtual void ReOrder(Order order)
        {
            ...
            foreach (var orderItem in _orderService.GetOrderItems(order.Id))
            {
                ...
                _shoppingCartService.AddToCart(user, product,
                    ...
                    orderItem.RentalStartDate, orderItem.RentalEndDate,
                    orderItem.Quantity, false);
            }

            //set checkout attributes
            //comment the code below if you want to disable this functionality
            _genericAttributeService.SaveAttribute(user, WCoreUserDefaults.CheckoutAttributes, order.CheckoutAttributesXml, order.StoreId);
        }
        ...
        public virtual bool IsReturnRequestAllowed(Order order)
        {
            ...
            var daysPassed = (DateTime.Now - order.CreatedOn).TotalDays;
        ...
        public virtual DateTime? GetNextPaymentDate(RecurringPayment recurringPayment)
        {
            ...
            if (historyCollection.Any())
            {
                ...
                {
                    RecurringProductCyclePeriod.Days => recurringPayment.StartDate.AddDays((double)recurringPayment.CycleLength * historyCollection.Count),
                    RecurringProductCyclePeriod.Weeks => recurringPayment.StartDate.AddDays((double)(7 * recurringPayment.CycleLength) * historyCollection.Count),
                    RecurringProductCyclePeriod.Months => recurringPayment.StartDate.AddMonths(recurringPayment.CycleLength * historyCollection.Count),
                    RecurringProductCyclePeriod.Years => recurringPayment.StartDate.AddYears(recurringPayment.CycleLength * historyCollection.Count),
                    _ => throw new WCoreException("Not supported cycle period"),
                };
            }
            else
            {
                if (recurringPayment.TotalCycles > 0)
                    result = recurringPayment.StartDate;
            }
