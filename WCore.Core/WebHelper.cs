...
namespace WCore.Core
{
    public class WebHelper : IWebHelper
    {
        ...
        public virtual SmtpException MailSender(string Body, string ErrorTitle = "Mail With Web Page", string ErrorMail = "")
        {
            var toMail = string.IsNullOrEmpty(ErrorMail) ? _mailSettings.EmailAddress : ErrorMail;

            if (string.IsNullOrEmpty(ErrorTitle))
            {
                ErrorTitle = "Mail With Web Page";
            }

            MailAddress addressFrom = new MailAddress(_mailSettings.EmailAddress, ErrorTitle);
            ...
            try
            {
                ...
                client.Host = _mailSettings.HostName;
                client.Port = _mailSettings.PortNumber;
                client.Credentials = new NetworkCredential(_mailSettings.EmailAddress, _mailSettings.EmailPassword);
                client.EnableSsl = _mailSettings.UseSSL;
                client.Send(message);
                return new SmtpException() { Source = "" };
            }
        ...
        public virtual string GetCurrentIpAddress()
        {
            ...
            try
            {
                ...
                if (_httpContextAccessor.HttpContext.Request.Headers != null)
                {
                    ...
                    var forwardedHttpHeaderKey = WCoreHttpDefaults.XForwardedForHeader;
        ...
        public virtual bool IsCurrentConnectionSecured()
        {
            ...
            if (_appSettings.HostingConfig.UseHttpClusterHttps)
                return _httpContextAccessor.HttpContext.Request.Headers[WCoreHttpDefaults.HttpClusterHttpsHeader].ToString().Equals("on", StringComparison.OrdinalIgnoreCase);

            //use HTTP_X_FORWARDED_PROTO?
            if (_appSettings.HostingConfig.UseHttpXForwardedProto)
                return _httpContextAccessor.HttpContext.Request.Headers[WCoreHttpDefaults.HttpXForwardedProtoHeader].ToString().Equals("https", StringComparison.OrdinalIgnoreCase);
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContextAccessor.HttpContext.Items[WCoreHttpDefaults.IsPostBeingDoneRequestItem] == null)
                    return false;

                return Convert.ToBoolean(_httpContextAccessor.HttpContext.Items[WCoreHttpDefaults.IsPostBeingDoneRequestItem]);
            }

            set => _httpContextAccessor.HttpContext.Items[WCoreHttpDefaults.IsPostBeingDoneRequestItem] = value;
        }
