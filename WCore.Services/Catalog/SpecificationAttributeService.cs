...
namespace WCore.Services.Catalog
{
    ...
    /// </summary>
    public partial class SpecificationAttributeService : Repository<SpecificationAttribute>, ISpecificationAttributeService
    {
        #region Fields
        ...
        public SpecificationAttributeService(WCoreContext context, ICacheKeyService cacheKeyService,
            IEventPublisher eventPublisher,
            IRepository<Product> productRepository,
            IRepository<ProductSpecificationAttribute> productSpecificationAttributeRepository,
            IRepository<SpecificationAttribute> specificationAttributeRepository,
            IRepository<SpecificationAttributeOption> specificationAttributeOptionRepository) : base(context)
        {
        ...
        public virtual IList<SpecificationAttribute> GetSpecificationAttributeByIds(int[] specificationAttributeIds)
        {
            ...
            var query = from p in context.SpecificationAttributes
        ...
        {
            var query = from sa in context.SpecificationAttributes
                        where context.SpecificationAttributeOptions.Any(o => o.SpecificationAttributeId == sa.Id)
                        select sa;

            return query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.SpecAttributesWithOptionsCacheKey));
        }
        ...
        public virtual IList<SpecificationAttributeOption> GetSpecificationAttributeOptionsBySpecificationAttribute(int specificationAttributeId)
        {
            ...
            var specificationAttributeOptions = query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.SpecAttributesOptionsCacheKey, specificationAttributeId));
        ...
        public virtual int[] GetNotExistingSpecificationAttributeOptions(int[] attributeOptionIds)
        {
            ...
            var query = context.SpecificationAttributeOptions;
            ...
            int specificationAttributeOptionId = 0, bool? allowFiltering = null, bool? showOnProductPage = null)
        {
            ...
            var key = _cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.ProductSpecificationAttributeAllByProductIdCacheKey,
                productId, specificationAttributeOptionId, allowFilteringCacheStr, showOnProductPageCacheStr);

            var query = _productSpecificationAttributeRepository.GetAll().AsQueryable();
            if (productId > 0)
        ...
        {
            var query = context.ProductSpecificationAttributes.AsQueryable();
            if (productId > 0)
                query = query.Where(psa => psa.ProductId == productId);
            if (specificationAttributeOptionId > 0)
                query = query.Where(psa => psa.SpecificationAttributeOptionId == specificationAttributeOptionId);
        ...
        public virtual IPagedList<Product> GetProductsBySpecificationAttributeId(int specificationAttributeId, int pageIndex, int pageSize)
        {
            var query = from product in context.Products
                        join psa in context.ProductSpecificationAttributes on product.Id equals psa.ProductId
                        join spao in context.SpecificationAttributeOptions on psa.SpecificationAttributeOptionId equals spao.Id
