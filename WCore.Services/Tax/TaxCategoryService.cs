...
namespace WCore.Services.Tax
{
    ...
    public partial class TaxCategoryService : ITaxCategoryService
    {
        ...
        {
            var query = from tc in _taxCategoryRepository.GetAll().AsQueryable()
                orderby tc.DisplayOrder, tc.Id
                select tc;

            var taxCategories = query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreTaxDefaults.TaxCategoriesAllCacheKey));
