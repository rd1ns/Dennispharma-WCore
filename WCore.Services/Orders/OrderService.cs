...
namespace WCore.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        public virtual Order GetOrderByCustomOrderNumber(string customOrderNumber)
        {
            ...
            return _orderRepository.GetAll().FirstOrDefault(o => o.CustomOrderNumber == customOrderNumber);
        }
        ...
        public virtual Order GetOrderByOrderItem(int orderItemId)
        {
            ...
            return (from o in _orderRepository.GetAll()
                    join oi in _orderItemRepository.GetAll() on o.Id equals oi.OrderId
            ...
            string orderNotes = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            ...
            if (warehouseId > 0)
            {
                ...
                query = from o in query
                    join oi in _orderItemRepository.GetAll() on o.Id equals oi.OrderId
                    join p in _productRepository.GetAll() on oi.ProductId equals p.Id
                    join pwi in _productWarehouseInventoryRepository.GetAll() on p.Id equals pwi.ProductId
                    where
            ...
            if (createdFrom.HasValue)
                query = query.Where(o => createdFrom.Value <= o.CreatedOn);

            if (createdTo.HasValue)
                query = query.Where(o => createdTo.Value >= o.CreatedOn);
            ...
            //    query = query.Where(o => _orderNoteRepository.GetAll().Any(oNote => oNote.OrderId == o.Id && oNote.Note.Contains(orderNotes)));

            query = from o in query
                join oba in _addressRepository.GetAll() on o.BillingAddressId equals oba.Id
                where
        ...
        public virtual Product GetProductByOrderItemId(int orderItemId)
        {
            ...
            return (from p in _productRepository.GetAll().AsQueryable()
                    join oi in _orderItemRepository.GetAll() on p.Id equals oi.ProductId
                    where oi.Id == orderItemId
        ...
        public virtual IList<OrderItem> GetOrderItems(int orderId, bool? isNotReturnable = null, bool? isShipEnabled = null, int vendorId = 0)
        {
            ...
            return (from oi in _orderItemRepository.GetAll()
                    join p in _productRepository.GetAll() on oi.ProductId equals p.Id
                    where
                    oi.OrderId == orderId &&
        ...
        public virtual IList<OrderItem> GetDownloadableOrderItems(int userId)
        {
            if (userId == 0)
            ...
            var query = from orderItem in _orderItemRepository.GetAll()
                        ...
                        p.IsDownload &&
                        !o.Deleted
                        orderby o.CreatedOn descending, orderItem.Id
                        select orderItem;
        ...
        /// <returns>Result</returns>
        public virtual IList<OrderNote> GetOrderNotesByOrderId(int orderId, bool? displayToUser = null)
        {
            if (orderId == 0)
                return new List<OrderNote>();

            var query = _orderNoteRepository.GetAll().Where(on => on.OrderId == orderId);
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            var query1 = from rp in _recurringPaymentRepository.GetAll()
                         join o in _orderRepository.GetAll() on rp.InitialOrderId equals o.Id
                         join c in _userRepository.GetAll() on o.UserId equals c.Id
                         where
                         !rp.Deleted &&
            ...
            var query2 = from rp in _recurringPaymentRepository.GetAll().AsQueryable()
                         where query1.Contains(rp.Id)
                         orderby rp.StartDate, rp.Id
                         select rp;
        ...
        public virtual IList<RecurringPaymentHistory> GetRecurringPaymentHistory(RecurringPayment recurringPayment)
        {
            ...
            return _recurringPaymentHistoryRepository.GetAll().Where(rph => rph.RecurringPaymentId == recurringPayment.Id).ToList();
        }
