...
namespace WCore.Services.Affiliates
{
    /// <summary>
    /// Affiliate service
    /// </summary>
    public partial class AffiliateService : Repository<Affiliate>, IAffiliateService
    {
        #region Fields
        ...
        public AffiliateService(WCoreContext context, IAddressService addressService,
        ...
        /// <param name="ordersCreatedFrom">Orders created date from (UTC); null to load all records. It's used only with "loadOnlyWithOrders" parameter st to "true".</param>
        /// <param name="ordersCreatedTo">Orders created date to (UTC); null to load all records. It's used only with "loadOnlyWithOrders" parameter st to "true".</param>
        ...
        public virtual IPagedList<Affiliate> GetAllAffiliates(string friendlyUrlName = null,
            ...
            DateTime? ordersCreatedFrom = null, DateTime? ordersCreatedTo = null,
            int pageIndex = 0, int pageSize = int.MaxValue,
            bool showHidden = false)
        {
            var query = context.Affiliates.AsQueryable();
            ...
            if (!string.IsNullOrWhiteSpace(firstName))
                query = from aff in query
                        join addr in context.Addresses.AsQueryable() on aff.AddressId equals addr.Id
            ...
            if (loadOnlyWithOrders)
            {
                ...
                if (ordersCreatedFrom.HasValue)
                    ordersQuery = ordersQuery.Where(o => ordersCreatedFrom.Value <= o.CreatedOn);
                if (ordersCreatedTo.HasValue)
                    ordersQuery = ordersQuery.Where(o => ordersCreatedTo.Value >= o.CreatedOn);
        ...
        public virtual string GenerateUrl(Affiliate affiliate)
        {
            ...
            var url = !string.IsNullOrEmpty(affiliate.FriendlyUrlName) ?
                //use friendly URL
                _webHelper.ModifyQueryString(storeUrl, WCoreAffiliateDefaults.AffiliateQueryParameter, affiliate.FriendlyUrlName) :
                //use ID
                _webHelper.ModifyQueryString(storeUrl, WCoreAffiliateDefaults.AffiliateIdQueryParameter, affiliate.Id.ToString());
        ...
        public virtual string ValidateFriendlyUrlName(Affiliate affiliate, string friendlyUrlName)
        {
            ...
            friendlyUrlName = CommonHelper.EnsureMaximumLength(friendlyUrlName, WCoreAffiliateDefaults.FriendlyUrlNameLength);
