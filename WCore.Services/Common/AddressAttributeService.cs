...
namespace WCore.Services.Common
{
    ...
    public partial class AddressAttributeService : IAddressAttributeService
    {
        ...
        {
            var query = from aa in _addressAttributeRepository.GetAll().AsQueryable()
        ...
        {
            var key = _cacheKeyService.PrepareKeyForDefaultCache(WCoreCommonDefaults.AddressAttributeValuesAllCacheKey, addressAttributeId);

            var query = from aav in _addressAttributeValueRepository.GetAll().AsQueryable()
