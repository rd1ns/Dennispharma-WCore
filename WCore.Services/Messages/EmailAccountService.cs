...
namespace WCore.Services.Messages
{
    ...
    /// </summary>
    public partial class EmailAccountService : Repository<EmailAccount>, IEmailAccountService
    {
        #region Fields
        ...
        public EmailAccountService(WCoreContext context, ICacheKeyService cacheKeyService,
            IEventPublisher eventPublisher,
            IRepository<EmailAccount> emailAccountRepository) : base(context)
        {
        ...
        public virtual void DeleteEmailAccount(EmailAccount emailAccount)
        {
            ...
            if (GetAllEmailAccounts().Count == 1)
                throw new WCoreException("You cannot delete this email account. At least one account is required.");
        ...
        {
            var query = from ea in _emailAccountRepository.GetAll().AsQueryable()
                        orderby ea.Id
                        select ea;

            var emailAccounts = query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreMessageDefaults.EmailAccountsAllCacheKey));
