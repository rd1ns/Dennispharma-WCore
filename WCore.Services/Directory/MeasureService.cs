...
namespace WCore.Services.Directory
{
    ...
    public partial class MeasureService : IMeasureService
    {
        ...
        {
            var query = from md in _measureDimensionRepository.GetAll().AsQueryable()
                        orderby md.DisplayOrder, md.Id
                        select md;
            var measureDimensions = query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreDirectoryDefaults.MeasureDimensionsAllCacheKey));
            ...
            MeasureDimension sourceMeasureDimension)
        {
            ...
            if (exchangeRatio == decimal.Zero)
                throw new WCoreException($"Exchange ratio not set for dimension [{sourceMeasureDimension.Name}]");
            ...
            MeasureDimension targetMeasureDimension)
        {
            ...
            if (exchangeRatio == decimal.Zero)
                throw new WCoreException($"Exchange ratio not set for dimension [{targetMeasureDimension.Name}]");
            result = result * exchangeRatio;
        ...
        {
            var query = from mw in _measureWeightRepository.GetAll().AsQueryable()
                        orderby mw.DisplayOrder, mw.Id
                        select mw;
            var measureWeights = query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(WCoreDirectoryDefaults.MeasureWeightsAllCacheKey));
        ...
        public virtual decimal ConvertToPrimaryMeasureWeight(decimal value, MeasureWeight sourceMeasureWeight)
        {
            ...
            if (exchangeRatio == decimal.Zero)
                throw new WCoreException($"Exchange ratio not set for weight [{sourceMeasureWeight.Name}]");
            result = result / exchangeRatio;
            ...
            MeasureWeight targetMeasureWeight)
        {
            ...
            if (exchangeRatio == decimal.Zero)
                throw new WCoreException($"Exchange ratio not set for weight [{targetMeasureWeight.Name}]");
            result = result * exchangeRatio;
