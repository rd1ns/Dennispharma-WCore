...
namespace WCore.Services.Plugins
{
    ...
    public partial class PluginService : IPluginService
    {
        ...
        protected virtual bool FilterByUser(PluginDescriptor pluginDescriptor, User user)
        {
            ...
            //return pluginDescriptor.LimitedToUserRoles.Intersect().Any();
            return true;
        }
        ...
        public virtual string GetPluginLogoUrl(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoExtension = WCorePluginDefaults.SupportedLogoImageExtensions
                .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{WCorePluginDefaults.LogoFileName}.{ext}")));
            if (string.IsNullOrWhiteSpace(logoExtension))
                return null;

            var storeLocation = _webHelper.GetStoreLocation();
            var logoUrl = $"{storeLocation}{WCorePluginDefaults.PathName}/" +
                $"{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{WCorePluginDefaults.LogoFileName}.{logoExtension}";
            ...
            if (checkDependencies)
            {
                ...
                if (descriptor.DependsOn?.Any() ?? false)
                {
                    ...
                    if (dependsOn.Any())
                    {
                        ...
                        throw new WCoreException(errorMessage);
                    }
                }
            }
        ...
        public virtual void InstallPlugins()
        {
            ...
            foreach (var descriptor in pluginDescriptors.OrderBy(pluginDescriptor => pluginDescriptor.DisplayOrder))
            {
                try
                {
                    ...
                    //activity log
                    //var user = _userService.GetById(0);
