    ...
    public partial class ReturnRequestModelFactory : IReturnRequestModelFactory
    {
        ...
        public virtual ReturnRequestListModel PrepareReturnRequestListModel(ReturnRequestSearchModel searchModel)
        {
            ...
            var startDateValue = !searchModel.StartDate.HasValue ? null
                : (DateTime?)_dateTimeHelper.ConvertToTime(searchModel.StartDate.Value, _dateTimeHelper.CurrentTimeZone);
            var endDateValue = !searchModel.EndDate.HasValue ? null
                : (DateTime?)_dateTimeHelper.ConvertToTime(searchModel.EndDate.Value, _dateTimeHelper.CurrentTimeZone).AddDays(1);
            ...
            var returnRequests = _returnRequestService.SearchReturnRequests(customNumber: searchModel.CustomNumber,
                ...
                createdFrom: startDateValue,
                createdTo: endDateValue,
            ...
            var model = new ReturnRequestListModel().PrepareToGrid(searchModel, returnRequests, () =>
            {
                return returnRequests.Select(returnRequest =>
                {
                    ...
                    returnRequestModel.CreatedOn = _dateTimeHelper.ConvertToUserTime(returnRequest.CreatedOn, DateTimeKind.Local);
            ...
            ReturnRequest returnRequest, bool excludeProperties = false)
        {
            ...
            model.CreatedOn = _dateTimeHelper.ConvertToUserTime(returnRequest.CreatedOn, DateTimeKind.Local);

            model.UserInfo = _userService.IsRegistered(user)
                ? user.Email : _localizationService.GetResource("Admin.Users.Guest");
            model.UploadedFileGuid = _downloadService.GetById(returnRequest.UploadedFileId)?.DownloadGuid ?? Guid.Empty;
            var orderItem = _orderService.GetOrderItemById(returnRequest.OrderItemId);
            if (orderItem != null)
            {
                var order = _orderService.GetOrderById(orderItem.OrderId);
