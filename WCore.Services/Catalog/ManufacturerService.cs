...
namespace WCore.Services.Catalog
{
    ...
    /// </summary>
    public partial class ManufacturerService : Repository<Manufacturer>, IManufacturerService
    {
        #region Fields
        ...
        private readonly IRepository<DiscountAppliedToManufacturer> _discountAppliedToManufacturerRepository;
        ...
        public ManufacturerService(WCoreContext context, CatalogSettings catalogSettings,
            ICacheKeyService cacheKeyService,
            IUserService userService,
            IEventPublisher eventPublisher,
            IRepository<DiscountAppliedToManufacturer> discountAppliedToManufacturerRepository,
            ...
            IWorkContext workContext) : base(context)
        {
            ...
            _discountAppliedToManufacturerRepository = discountAppliedToManufacturerRepository;
        ...
        public virtual void ClearDiscountManufacturer(Discount discount)
        {
            if (discount is null)
                throw new ArgumentNullException(nameof(discount));

            var mappings = context.DiscountAppliedToManufacturers.Where(dcm => dcm.DiscountId == discount.Id);

            if (!mappings.Any())
                return;

            _discountAppliedToManufacturerRepository.BulkDelete(mappings.ToList());
        }
        ...
        public virtual IList<int> GetAppliedManufacturerIds(Discount discount, User user)
        {
            if (discount == null)
                throw new ArgumentNullException(nameof(discount));

            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(WCoreDiscountDefaults.DiscountManufacturerIdsModelCacheKey,
                discount,
                _workContext.CurrentUser.UserType,
                _storeContext.CurrentStore);

            var result = context.DiscountAppliedToManufacturers.Where(dmm => dmm.DiscountId == discount.Id)
        ...
        {
            var manufacturers = context.Set<Manufacturer>().AsQueryable();

            if (discountId.HasValue)
                manufacturers = from manufacturer in manufacturers
                                join dmm in _discountAppliedToManufacturerRepository.GetAll() on manufacturer.Id equals dmm.EntityId
        ...
        public virtual List<Manufacturer> GetManufacturersByIds(int[] manufacturerIds)
        {
            ...
            var query = from p in context.Manufacturers
            ...
            if (!showHidden && (!_catalogSettings.IgnoreAcl || !_catalogSettings.IgnoreStoreLimitations))
            {
                if (!_catalogSettings.IgnoreStoreLimitations)
                {
                    ...
                    query = from pm in query
                            join m in context.Manufacturers on pm.ManufacturerId equals m.Id
                            join sm in context.StoreMappings
                                on new
                                {
                                    c1 = m.Id,
                ...
                query = query.Distinct().OrderBy(pm => pm.DisplayOrder).ThenBy(pm => pm.Id).ToList();
            }

            var productManufacturers = new PagedList<ProductManufacturer>(query.ToList(), pageIndex, pageSize);
            ...
            bool showHidden = false)
        {
            ...
            var key = _cacheKeyService.PrepareKeyForDefaultCache(WCoreCatalogDefaults.ProductManufacturersAllByProductIdCacheKey, productId,
        ...
        {
            var query = context.ProductManufacturers;
        ...
        public DiscountAppliedToManufacturer GetDiscountAppliedToManufacturer(int manufacturerId, int discountId)
        {
            return context.DiscountAppliedToManufacturers.FirstOrDefault(dcm => dcm.EntityId == manufacturerId && dcm.DiscountId == discountId);
        }
