...
namespace WCore.Services.Seo
{
    public class UrlRecordService : Repository<UrlRecord>, IUrlRecordService
    {
        ...
        public UrlRecordService(WCoreContext context,
            ILanguageService languageService,
            IStaticCacheManager staticCacheManager,
            ICacheKeyService cacheKeyService,
            IWorkContext workContext,
            LocalizationSettings localizationSettings,
            SeoSettings seoSettings) : base(context)
        {
        ...
        {
            return context.Set<UrlRecord>().Where(o => urlRecordIds.Contains(o.Id)).ToList();
        }
        ...
        {
            return GetById(urlRecordId, cache => default);
        }
        ...
        public virtual UrlRecord GetBySlug(string slug)
        {
            ...
            var key = _cacheKeyService.PrepareKeyForDefaultCache(WCoreSeoDefaults.UrlRecordBySlugCacheKey, slug);
            ...
            //gradual loading
            var query = context.UrlRecords.Where(o => o.Slug == slug).OrderBy(o => o.IsActive).ThenByDescending(o => o.Id);
        ...
        {
            var urlRecords = context.Set<UrlRecord>().OrderBy(ur => ur.Slug).AsQueryable();
        ...
        {
            entityName = entityName.Replace("Proxy", "");
            //gradual loading
            var key = _cacheKeyService.PrepareKeyForDefaultCache(WCoreSeoDefaults.UrlRecordCacheKey, entityId, entityName, languageId);
            ...
            var query = context.Set<UrlRecord>().Where(o => o.EntityId == entityId && o.EntityName == entityName && o.LanguageId == languageId && o.IsActive).OrderByDescending(o => o.Id).Select(o => o.Slug);
        ...
        public virtual void SaveSlug<T>(T entity, string slug, int languageId) where T : BaseEntity, ISlugSupported
        {
            ...
            var query = context.Set<UrlRecord>().Where(o => o.EntityId == entityId && o.EntityName == entityName && o.LanguageId == languageId).OrderByDescending(o => o.Id);
            ...
            bool returnDefaultValue = true, bool ensureTwoPublishedLanguages = true)
        {
            ...
            if (languageId > 0)
            {
                ...
                {
                    var totalPublishedLanguages = _languageService.GetAllLanguages(null,null).Count;
        ...
        public virtual string GetSeName(string name, bool convertNonWesternChars, bool allowUnicodeCharsInUrls)
        {
            ...
            var okChars = "abcdefghijklmwCoreqrstuvwxyz1234567890 _-";
        ...
        public virtual string ValidateSeName(int entityId, string entityName, string seName, string name, bool ensureNotEmpty)
        {
            ...
            //max length
            seName = CommonHelper.EnsureMaximumLength(seName, WCoreSeoDefaults.SearchEngineNameLength);
            ...
            while (true)
            {
                ...
                var reserved2 = _seoSettings.ReservedUrlRecordSlugs.Contains(tempSeName);
                //and it's not equal to a language code
                var reserved3 = _languageService.GetAllLanguages(null,null).Any(language => language.UniqueSeoCode.Equals(tempSeName, StringComparison.InvariantCultureIgnoreCase));
