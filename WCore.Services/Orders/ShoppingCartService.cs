...
namespace WCore.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
            ...
            DateTime? rentalEndDate)
        {
            ...
            //rental products
            var rentalInfoEqual = shoppingCartItem.RentalStartDate == rentalStartDate && shoppingCartItem.RentalEndDate == rentalEndDate;
        ...
        {
            return !_sciRepository.GetAll().AsQueryable().Any(sci => sci.UserId == user.Id);
        }
            ...
            bool ensureOnlyActiveCheckoutAttributes = false)
        {
                ...
                shoppingCartItem.ShoppingCartType == ShoppingCartType.ShoppingCart)
            {
                ...
                var checkoutAttributesXml =
                    _genericAttributeService.GetAttribute<string>(user, WCoreUserDefaults.CheckoutAttributes,
                        storeId);
                checkoutAttributesXml =
                    _checkoutAttributeParser.EnsureOnlyActiveAttributes(checkoutAttributesXml, cart);
                _genericAttributeService.SaveAttribute(user, WCoreUserDefaults.CheckoutAttributes,
            ...
            var requiredShoppingCartItems =
                GetShoppingCart(user, shoppingCartType: shoppingCartItem.ShoppingCartType)
        ...
        public virtual void DeleteShoppingCartItem(int shoppingCartItemId, bool resetCheckoutData = true,
            bool ensureOnlyActiveCheckoutAttributes = false)
        {
            var shoppingCartItem = _sciRepository.GetAll().FirstOrDefault(sci => sci.Id == shoppingCartItemId);
            if (shoppingCartItem != null)
        ...
        public virtual int DeleteExpiredShoppingCartItems(DateTime olderThan)
        {
            ...
            int quantity)
        {
            ...
            if (product.AvailableStartDateTime.HasValue)
            {
                var availableStartDateTime = DateTime.SpecifyKind(product.AvailableStartDateTime.Value, DateTimeKind.Local);
                if (availableStartDateTime.CompareTo(DateTime.Now) > 0)
                {
            ...
            if (!product.AvailableEndDateTime.HasValue || availableStartDateError)
                return warnings;

            var availableEndDateTime = DateTime.SpecifyKind(product.AvailableEndDateTime.Value, DateTimeKind.Local);
            if (availableEndDateTime.CompareTo(DateTime.Now) < 0)
            {
                warnings.Add(_localizationService.GetResource("ShoppingCart.NotAvailable"));
            }
        ...
        /// <param name="createdFrom">Created date from (UTC); pass null to load all records</param>
        /// <param name="createdTo">Created date to (UTC); pass null to load all records</param>
        /// <returns>Shopping Cart</returns>
        public virtual IList<ShoppingCartItem> GetShoppingCart(User user, ShoppingCartType? shoppingCartType = null,
            int storeId = 0, int? productId = null, DateTime? createdFrom = null, DateTime? createdTo = null)
        {
            if (user == null)
                throw new ArgumentNullException(nameof(user));

            var items = _sciRepository.GetAll().Where(sci => sci.UserId == user.Id);
            ...
            if (createdFrom.HasValue)
                items = items.Where(item => createdFrom.Value <= item.CreatedOn);
            if (createdTo.HasValue)
                items = items.Where(item => createdTo.Value >= item.CreatedOn);

            var key = _cacheKeyService.PrepareKeyForShortTermCache(WCoreOrderDefaults.ShoppingCartCacheKey, user, shoppingCartType, storeId, productId, createdFrom, createdTo);
            ...
            bool ignoreConditionMet = false)
        {
            ...
            //var attributes1 = _productAttributeParser.ParseProductAttribues(attributesXml);
            ...
            foreach (var attributeValue in attributeValues)
            {
                ...
                var productAttribute = _productAttributeService.GetProductAttributeById(attributeValue.ProductProductAttributeId);

                if (ignoreNonCombinableAttributes && productAttribute != null/* && productAttribute.IsNonCombinable()*/)
                    continue;
            ...
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null)
        {
            ...
            var todayDtUtc = _dateTimeHelper.ConvertToTime(todayDt, _dateTimeHelper.DefaultStoreTimeZone);
            //dates are entered in store timezone (e.g. like in hotels)
            var startDate = _dateTimeHelper.ConvertToTime(rentalStartDate.Value, _dateTimeHelper.DefaultStoreTimeZone);
            //but we what if dates should be entered in a user timezone?
            //DateTime startDate = _dateTimeHelper.ConvertToTime(rentalStartDate.Value, _dateTimeHelper.CurrentTimeZone);
            if (todayDt.CompareTo(startDate) <= 0)
                return warnings;
            ...
            out List<Discount> appliedDiscounts)
        {
            ...
            return GetUnitPrice(product,
                ...
                shoppingCartItem.RentalStartDate,
                shoppingCartItem.RentalEndDate,
                includeDiscounts,
                out discountAmount,
            ...
            int quantity = 1, bool addRequiredProducts = true)
        {
            ...
            if (shoppingCartItem != null)
            {
                ...
                shoppingCartItem.UpdatedOn = DateTime.Now;
            ...
            int quantity = 1, bool resetCheckoutData = true)
        {
            ...
            if (quantity > 0)
            {
                ...
                shoppingCartItem.RentalStartDate = rentalStartDate;
                shoppingCartItem.RentalEndDate = rentalEndDate;
                shoppingCartItem.UpdatedOn = DateTime.Now;
        ...
        public virtual void MigrateShoppingCart(User fromUser, User toUser, bool includeCouponCodes)
        {
            ...
            for (var i = 0; i < fromCart.Count; i++)
            {
                ...
                AddToCart(toUser, product, sci.ShoppingCartType, sci.StoreId,
                    ...
                    sci.RentalStartDate, sci.RentalEndDate, sci.Quantity, false);
            }
            ...
            var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(fromUser, WCoreUserDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
            _genericAttributeService.SaveAttribute(toUser, WCoreUserDefaults.CheckoutAttributes, checkoutAttributesXml, _storeContext.CurrentStore.Id);
        }
            ...
            out int cycleLength, out RecurringProductCyclePeriod cyclePeriod, out int totalCycles)
        {
            ...
            foreach (var sci in shoppingCart)
            {
                ...
                if (product == null)
                {
                    throw new WCoreException($"Product (Id={sci.ProductId}) cannot be loaded");
                }
